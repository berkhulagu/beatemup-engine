...
package org.bh.robotlegs.mvc.model.components.character
{
	...
	public class CharacterBean
	{
		...
		public function calculateFollow():void {
			...
			if(_targetCharacter.posX > posX && isFacingLeft) {
				...
				if(_targetCharacter.posX < posX && isFacingRight) {
					facing += Math.PI
				} 
			
			if( (_targetCharacter.posX > posX && _targetCharacter.posX < posX +  Math.abs(_targetOffset.x)) ||
				(_targetCharacter.posX < posX && _targetCharacter.posX > posX -  Math.abs(_targetOffset.x)))
			{
				
				if(tTargetY < posY && tTargetY + _targetOffset.y > posY ||
				   tTargetY > posY && tTargetY - _targetOffset.y < posY)
				{
					_stop = true 
					return;
				} else {
