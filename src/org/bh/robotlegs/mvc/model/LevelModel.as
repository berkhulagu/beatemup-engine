package org.bh.robotlegs.mvc.model
{
	...
	import org.bh.robotlegs.events.ConversationEndedEvent;
	...
	import org.bh.robotlegs.mvc.model.components.hotspots.actions.AddEnemyAction;
	import org.bh.robotlegs.mvc.model.components.hotspots.actions.IAction;
	import org.bh.robotlegs.mvc.model.components.hotspots.actions.MoveHotspotAction;
	import org.bh.robotlegs.mvc.model.components.hotspots.actions.SetCameraAction;
	import org.bh.robotlegs.mvc.model.components.hotspots.actions.SetWalkLimitAction;
	import org.bh.robotlegs.mvc.model.components.level.CloneInfo;
	import org.bh.robotlegs.mvc.model.components.level.LayerInfo;
	import org.bh.robotlegs.mvc.model.components.level.postsimulation.IPostSimulation;
	import org.bh.robotlegs.mvc.model.components.level.postsimulation.LimitCharacterMovementDecorator;
	import org.bh.robotlegs.mvc.model.components.level.postsimulation.PostSimulation;
	...
	import org.bh.robotlegs.mvc.model.components.vo.LevelVO;
	...
	public class LevelModel extends Actor
	{
		...
		private var _gamestate:int = 1; // 1. playing 2. dead
			...
			for each(var tCharacterBean:CharacterBean in characterBeans) {
				...
				if(tCharacterBean.id != "avatar") {
					...
					if(tCharacterBean.conversationId != null && tCharacterBean.conversationId != "") {
						...
						if(Math.abs(_avatarsCharacterBean.posX - tCharacterBean.posX) < 150 &&
						   Math.abs(_avatarsCharacterBean.posY - tCharacterBean.posY) < 150)
						{
			...
			switch(tCheck.command) {
				...
				case "avatar_passed_hotspot":
					...
					if(_insideHotspotsList[int(tCheck.value)]) { 
						_insideHotspotsList[int(tCheck.value)] = null //HACK!
						tRetVal = true
					}
					break;
		...
		private function performBehaviours(tCharacterBean:CharacterBean, tStateManagerId:int, tStateId:int, tUpdateVisuals:Boolean):void
		{
			...
			for each(var tBehaviour:IBehaviour in tState.behaviours) {
				...
				if(tBehaviour is WalkLimitBehaviour) {
				...
				if(tBehaviour is LogBehaviour) {
					var tLogBehaviour:LogBehaviour = tBehaviour as LogBehaviour
						
					trace("LOG> " + tLogBehaviour.text)
				} else
				if(tBehaviour is AddCharacterBehaviour) {
					var tAddCharacterBehaviour:AddCharacterBehaviour = tBehaviour as AddCharacterBehaviour
					
					var tCharacterBean:CharacterBean =  new CharacterBean( 
						tAddCharacterBehaviour.enemyId,
						tAddCharacterBehaviour.enemyType,
						tAddCharacterBehaviour.stateManagerId,
						tAddCharacterBehaviour.initialState,
						tAddCharacterBehaviour.posX,
						tAddCharacterBehaviour.posY, "", 0)
				...
				if(tBehaviour is SetConversationBehaviour) {
				...
				if(tBehaviour is AddConversationListenerBehaviour) {
				...
				if(tBehaviour is UpdateCharacterStateBehaviour) {
				...
				if(tBehaviour is DealDamageBehaviour) {
				...
				if(tBehaviour is FollowBehaviour) {
					...
					if(tBehaviour is HealthBehaviour) {
		...
		private function triggerAction(tActions:IAction):void {
			...
			if(tActions is AddEnemyAction) {
				var tAddEnemyAction:AddEnemyAction = tActions as AddEnemyAction
				var tCharacterBean:CharacterBean =  new CharacterBean( 
					tAddEnemyAction.enemyId,
					tAddEnemyAction.enemyType,
					tAddEnemyAction.stateManagerId,
					tAddEnemyAction.initialState,
					tAddEnemyAction.parent.x + tAddEnemyAction.distanceX,
					tAddEnemyAction.parent.y + tAddEnemyAction.distanceY, "", 0)
			...
			if(tActions is SetCameraAction) {
			...
			if(tActions is SetWalkLimitAction) {
		...
		private function handleKeyStrokes(tKeyPoll:KeyPoll):void {
			...
			if(tKeyPoll.isDown( Keyboard.RIGHT ) || 
			   tKeyPoll.isDown( Keyboard.LEFT )  ||
			   tKeyPoll.isDown( Keyboard.UP ) 	 ||
			   tKeyPoll.isDown( Keyboard.DOWN )) {
					...
					if(!_isInteracting) {
						...
						if(_avatarsCharacterBean.currentAnimation != "JumpUpdate_01" && 
							_avatarsCharacterBean.currentAnimation != "Melee")	
				...
				if(_isResponseTurn && 
				  (tKeyPoll.isDown( Keyboard.NUMBER_1) ||
				   tKeyPoll.isDown( Keyboard.NUMBER_2) ||
				   tKeyPoll.isDown( Keyboard.NUMBER_3) ||
				   tKeyPoll.isDown( Keyboard.NUMBER_4))) {
			...
			for each(var tCharacterBean:CharacterBean in _characterBeans) {
				if(tCharacterBean.id != "avatar") {
					...
					if(Math.abs(tDistanceX) < 150 && Math.abs(tDistanceY) < 30) {
						// facing ?
						if(tDistanceX < 0 && _avatarsCharacterBean.isFacingRight ||
						   tDistanceX > 0 && _avatarsCharacterBean.isFacingLeft) {
